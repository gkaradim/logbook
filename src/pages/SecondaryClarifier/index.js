...
import "./SecondaryClarifier.scss";
import "../style.scss";
import { ListItemAvatar, ListItemSecondaryAction } from "@material-ui/core";

const SecondaryClarifier = ({ setSecondaryClarifierData }) => {
  ...
  const getTodayValues = async () => {
    try {
      ...
      if (response.status === 200) {
        ...
        setSecondaryClarifierData(data);
  ...
  const changeDate = async (date) => {
    try {
      setDate(date);
      setData(null);
      setSecondaryClarifierData(null);
...
export default SecondaryClarifier;
